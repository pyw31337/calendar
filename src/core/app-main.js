...
function CalendarApp() {
  ...
  const {
    ...
    setOlderChatMessages, hasMoreOlderChat, loadingOlderChat, loadingOlderChatRef,
    allChatMessages, chatMessagesContainerRef, loadOlderChatMessages
  } = useChatMessageWindow({
    React, activeCalId, activeView, isInitialDataLoading,
    firebaseDb, getFirebaseDb: () => firebaseDb, firebaseConnectionVersion,
    chatParticipantIdRef,
    // activeCalRef is declared further down; only read from listener callbacks, after render.
    getActiveCal: () => activeCalRef.current
  });
  const { memos, setMemos, memosLimit, setMemosLimit, hasMoreMemos, setHasMoreMemos } = useMemoCollections({
  ...
  // (chatMessagesContainerRef and the older-history/gallery-live reset live in useChatMessageWindow.)
  React.useEffect(() => {
