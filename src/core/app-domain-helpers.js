...
function buildDynamicManifest(calendar) {
  ...
  return {
    ...
    theme_color: '#4F46E5',
    icons: [
      { src: `${base}icons/icon-v4-192.png`, sizes: '192x192', type: 'image/png', purpose: 'any' },
      { src: `${base}icons/icon-v4-512.png`, sizes: '512x512', type: 'image/png', purpose: 'any' },
      { src: `${base}icons/icon-v4-512-maskable.png`, sizes: '512x512', type: 'image/png', purpose: 'maskable' }
    ]
  };
}
...
const getCalendarSettlementCards = GATHER_APP_UTILS.getCalendarSettlementCards
  ? GATHER_APP_UTILS.getCalendarSettlementCards
  : function getCalendarSettlementCards(calendar) {
  return Array.isArray(calendar?.settlementCards) ? calendar.settlementCards.filter(sc => sc && !isTombstone(sc)) : [];
};
