...
## 2026-09-21 10:14 KST

- **신규 Tabler 아이콘 16종 등록·적용** (요청: 공지사항/정산생성/정산수정/입금/출금/이미지수정/이미지다운로드/이미지없음/이미지설정/이미지업로드/이미지검색/링크추가/링크/링크불가/컨텐츠등록/인물추가). 추측으로 path 데이터를 재구성하지 않고 공식 `@tabler/icons` CDN(jsdelivr)에서 outline/filled 원본 SVG를 직접 내려받아 `<path>` 데이터를 그대로 `src/ui/v2/tabler-icons.js`의 `TABLER_ICONS`에 추가(`speakerphone`, `cashPlus`, `cashBanknoteEdit`, `cashBanknoteMoveBack`, `cashBanknoteMove`, `photoEdit`, `photoOff`, `photoCog`, `photoUp`, `photoSearch`, `linkPlus`, `link`(off+filled on), `linkOff`, `scriptPlus`, `moodPlus`). `photoDown`은 이미 등록돼 있던 path가 공식 소스와 정확히 일치해 변경 없이 유지.
- 실제 적용: `src/ui/ui-app-shell-v2.js`의 `TAB_MENU_ITEM_CONFIGS`에서 기존에 부정확하거나 임시로 재사용 중이던 아이콘 키를 교체 — 공지사항 `more`→`speakerphone`, 정산 생성 `plus`→`cashPlus`, 이미지 업로드 `gallery`→`photoUp`, 컨텐츠 등록 `plus`→`scriptPlus`. 갤러리 링크 업로드 항목은 원래도 `icon:'link'`를 참조하고 있었지만 레지스트리에 `link` 키 자체가 없어 아무 아이콘도 렌더되지 않던 잠재 버그였는데, 이번에 `link`를 등록하면서 자연스럽게 해결됨.
- 정산 수정/입금/출금/이미지 수정·없음·설정·검색/링크 추가·불가/인물 추가는 코드베이스 전체를 검색한 결과 아이콘을 붙일 수 있는 기존 UI 슬롯(TabIcon 렌더 지점)이 없음을 확인 — 대부분 레거시 텍스트 전용 버튼(`ui-event-modals.js`의 "정산 수정" 등)이거나 아직 없는 기능이었음. 없는 슬롯에 억지로 아이콘 자리를 새로 만드는 대신, 이 파일에 이미 확립된 "당장 안 쓰는 아이콘도 잘 보관해 뒀다가 필요할 때 활용" 관례를 따라 레지스트리에 정확한 공식 path로 등록만 해두고 대기.
- 어드민단: `ui-admin-dashboard.js`/`ui-meme-admin.js`에 이미 있는 `makeAdminTablerIcon(iconKey, size)` 패턴을 확인했으나, 마찬가지로 위 신규 라벨에 대응하는 기존 아이콘 렌더 지점이 없어 적용 대상 없음. 레지스트리가 공유되는 단일 소스이므로 두 파일 모두 `TABLER_ICONS`를 그대로 import해 쓰고 있어, 추후 해당 액션이 추가되면 곧바로 재사용 가능.
- **사이드메뉴 접기/펼치기 검색 버튼 좌우 중앙정렬 수정**: `src/ui/v2/aurora-theme.css`의 `.bp-side-nav.bp-is-collapsed .bp-side-nav-global-search` 규칙(878-926줄대, 1239-1274줄대 "Final rail coordinate contract" 두 자리 모두)에 `gap: 0`을 추가. 원인은 펼침 상태의 `display:flex; gap:10px` 행(아이콘+라벨)이 접힘 상태에서 라벨을 `display:none`이 아니라 `max-width:0;opacity:0`으로만 숨기던 방식이라, `gap`이 여전히 아이콘과 (보이지 않는) 라벨 사이 공간을 차지해 돋보기 아이콘이 원형 버튼 중앙에서 좌측으로 치우쳐 보이던 것. `gap:0`으로 그 잔여 여백을 제거해 아이콘이 버튼 중앙에 오도록 수정. 상단 로고(`bp-side-nav-brand-icon`)는 이미 접힘/펼침 상태 구분 없는 공통 규칙(1269-1273줄)이 26px 고정 크기를 강제하고 있어 x,y가 상태와 무관하게 고정되어 있음을 코드로 확인.
- 이 파일에는 동일한 "side-nav coordinate contract"를 다루는 중복/파편화된 CSS 블록이 6곳 이상 남아 있음(이전 세션에서 발견, 문서화됨). 이번 수정은 그 중 실제로 값이 적용되는 2곳(878번대, 1239번대)에만 최소 변경으로 반영했고, 나머지 중복 블록 전체를 하나로 통합하는 대규모 리팩터는 회귀 위험이 커서 이번 유닛에서는 진행하지 않음 — 다음 유닛 후보로 남겨둠.
- 검증: 이번 세션 Firebase 네트워크가 샌드박스에서 차단돼 `vite preview`가 로그인 데이터 없이 "불러오는 중…" 상태에 멈춰 Playwright로 실사용 좌표를 직접 측정하지 못함(격리된 정적 HTML 하네스로도 재현 시도했으나 프리뷰 서버의 SPA 라우팅 때문에 신뢰할 수 있는 측정에 실패). 대신 `gap` 속성이 flex 자식 요소 간 실제 렌더 폭과 무관하게 항상 간격을 차지한다는 CSS 명세상 확실한 사실에 근거해 수정했으며, 코드 정적 검토로 다른 winning 규칙과 충돌하지 않음을 확인. **라이브에서 재확인이 필요함.**
