...
- 유닛: V2 기본 전환 준비 및 계산 스타일 회귀 게이트
- 수정: 기본 셸은 여전히 V1로 유지하되, 승인 시 GitHub 저장소 변수 `VITE_DEFAULT_SHELL=v2`만으로 V2 기본 배포가 가능하도록 전환 플래그를 보완. `?shell=v1`은 기본 V2 배포 뒤에도 V1을 강제하는 롤백 경로로 유지.
- 수정: V2 메모 이미지의 1:1 규칙을 지연 로드 스타일에만 두지 않고 공통 메모 카드 모듈에 추가하여 로딩 순서와 관계없이 `aspect-ratio: 1 / 1; object-fit: cover`가 적용되게 함.
- 검증: V1/V2 메모 카드·태그·PC/모바일 그리드·단어 줄바꿈·이미지 비율의 계산 스타일을 브라우저 스모크에 추가. V2 전환 런북과 로컬 `check:v2-cutover` 게이트를 추가.
- 데이터 안전성: Firebase·Storage 읽기/쓰기 모델, 공유 URL, 기존 V1 기본 배포는 변경하지 않음.
