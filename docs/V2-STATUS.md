...
최종 갱신: 2026-09-23 16:05 KST (Phase 3 app.css chat/memo shell hardcodes)

## 상태

- **기본 URL 컷오버: 아직 하지 않음** — `?id=cw` 는 V1, V2는 `?shell=v2` 옵트인.
- 다음 작업: **다크 Phase 3** — `app.css` `.renewal-shell-main.is-chat/.is-records` 라이트 `!important` 제거(#739 예정). QA: 채팅/메모/홈 카드 다크 재검증.
...
| 다크모드 | **P0 Phase 3** — shell canvas OK; chat/memo `app.css` hardcodes + 홈 카드 QA 중 |
