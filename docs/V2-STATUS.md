...
최종 갱신: 2026-09-23 (Claude, `main` @ `7dd6d374` 기준, 다크 Phase4 픽스 완료 — PR 대기)

## 상태

- **기본 URL 컷오버: 아직 하지 않음** — `?id=cw` 는 V1, V2는 `?shell=v2` 옵트인.
- **다크 Phase1–3 코드 머지됨** (#736/#737/#738/#739). **Phase4는 로컬 검증 완료, PR 대기** — `src/app.css`의 `.renewal-shell` 블록이 `--renewal-bg/card/text/muted`를 하드코딩 라이트 값으로 재정의해 모든 V2 페이지 헤더 타이틀·카드 제목이 다크에서 거의 안 보이던 근본 원인을 잡음. 다음: **Phase4 PR 머지 → 라이브 재QA (모달·토스트 포함) → Safari 채팅 VV**.
...
| 다크모드 | **P0 Phase1–3 머지, Phase4 로컬 검증 완료 (PR 대기)** — 헤더 타이틀/카드 제목 근본 원인 잡음, 라이브 재QA·모달/토스트 확인 남음 |
| Safari 채팅 키보드 / visualViewport | **P0 미검증** |
| 기본 shell 플래그 전환 | **금지** (다크·사파리 후 + `?shell=v1` 한 릴리스) |

## 최근 머지 (발췌)

- *(PR 대기)* 다크 Phase4 — `.renewal-shell` `--renewal-*` 변수 재하드코딩 근본원인 수정 + `src/ui/v2` 잔여 리터럴 `#1e1b2e` 정리
