...
**최종 갱신:** 2026-09-23 (Claude, `main` @ `7dd6d374` 기준, 다크 Phase4 로컬 검증 완료 — PR 대기)  
**상태 한 줄:** **아직 기본 주소를 V2로 바꾸지 말 것.** V2는 `?shell=v2` 옵트인. 다크 토큰 Phase1–3 머지됨 + Phase4(근본원인: `.renewal-shell` 변수 재하드코딩)는 로컬 검증 완료로 PR 대기. 다음: **Phase4 PR 머지 → 라이브 재QA(모달·토스트 포함) → Safari 채팅 VV**. 다크 상세: [`docs/v2-dark-mode-handoff.md`](./v2-dark-mode-handoff.md).
...
6. 다음 착수 유닛: **다크 Phase4 PR 머지 → 라이브 재QA** (Phase1–3 머지됨, Phase4는 헤더 타이틀·카드 제목이 안 보이던 근본원인을 로컬에서 잡고 PR 대기 중). 상세 [`v2-dark-mode-handoff.md`](./v2-dark-mode-handoff.md). 새 토글 금지 — `themeChoice` 유지. 그다음 Safari 채팅 VV.
...
| 4 `.renewal-shell` `--renewal-*` 변수 재하드코딩 근본원인 + `src/ui/v2` 잔여 리터럴 `#1e1b2e` | *(PR 대기)* | 로컬 검증 완료 |
...
1. Phase 4 PR 머지 → 라이브 재QA (채팅/메모/장소/정산/갤러리/컨텐츠/보관함 8개 화면, 이번엔 텍스트 색까지 확인 — 로컬 빌드로는 검증했지만 라이브 배포본 재확인 필요).
